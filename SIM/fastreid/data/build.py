...
from methods.SIM.fastreid.config import configurable
from methods.SIM.fastreid.utils import comm
...
def _dataset_kwargs_from_cfg(cfg, dataset_name):
    if dataset_name == "CSG":
        return {
            "dataset_root": cfg.DATASETS.CSG_ROOT,
            "image_root": cfg.DATASETS.CSG_IMAGE_ROOT,
            "label_root": cfg.DATASETS.CSG_LABEL_ROOT,
            "enhanced_root": cfg.DATASETS.CSG_ENHANCED_ROOT,
        }
    if dataset_name == "RoadGroup":
        return {
            "dataset_root": cfg.DATASETS.ROADGROUP_ROOT,
            "image_root": cfg.DATASETS.ROADGROUP_IMAGE_ROOT,
            "annotation_root": cfg.DATASETS.ROADGROUP_ANNOTATION_ROOT,
            "enhanced_path": cfg.DATASETS.ROADGROUP_ENHANCED_PATH,
        }
    if dataset_name == "DukeGroup":
        return {
            "dataset_root": cfg.DATASETS.DUKEGROUP_ROOT,
            "image_root": cfg.DATASETS.DUKEGROUP_IMAGE_ROOT,
            "label_root": cfg.DATASETS.DUKEGROUP_LABEL_ROOT,
        }
    return {}
...
def _train_loader_from_config(cfg,
    ...
    if train_set is None:
        ...
        for d in cfg.DATASETS.NAMES:
            data = DATASET_REGISTRY.get(d)(
                root=_root, **_dataset_kwargs_from_cfg(cfg, d), **kwargs
            )
...
def _test_loader_from_config(cfg,
    ...
    if test_set is None:
        ...
        data = DATASET_REGISTRY.get(dataset_name)(
            root=_root, **_dataset_kwargs_from_cfg(cfg, dataset_name), **kwargs
        )
        if comm.is_main_process():
            data.show_test()
...
def fast_batch_collator(batched_inputs):
    ...
    if isinstance(elem, torch.Tensor):
        out = torch.zeros((len(batched_inputs), *elem.size()),
                          dtype=elem.dtype)
        ...
        out = {}
        for key in elem:
            if key in ['images_p', 'targets_p', 'layout', 'avg_probs']:
                # pdb.set_trace()
                out[key] = torch.cat([x[key] for x in batched_inputs], dim=0)
            else:
                out[key] = fast_batch_collator(
