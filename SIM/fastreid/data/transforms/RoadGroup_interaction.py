...
def load_roadgroup_data(annotation_dir):
    """加载RoadGroup数据集并转换为CSG格式"""
    # 加载三个JSON文件
    ...
    # 创建person id映射表
    global_pid = 1
    img_pid_to_global = defaultdict(dict)  # {img_name: {local_pid: global_pid}}

    # 处理person_correspondance.json建立全局person id映射
    for correspondance in correspondance_data:
        group_pair = correspondance['group_pair']
        person_pairs = correspondance['person pairs']

        # 为这个group pair创建全局person id
        for pair in person_pairs:
            pid1 = pair['person1 id']
            pid2 = pair['person2 id']
            img1 = group_pair[0]
            img2 = group_pair[1]
            ...
            if pid1 not in img_pid_to_global[img1]:
                ...
                # 如果第二个图像中的person id对应第一个图像，则使用相同的全局id
...
def process_roadgroup_dataset(image_root, annotation_dir, pt_matrix_path, output_dir):
    # 确保输出目录存在
    os.makedirs(output_dir, exist_ok=True)

    # 修改输出路径为输出目录下的文件名
    output_path = osp.join(output_dir, 'RoadGroup_enhanced.pkl')

    # 加载透视变换矩阵
    pt_matrix = np.load(pt_matrix_path)

    # 加载并转换RoadGroup数据
    img_names, gids, pids, all_bboxes = load_roadgroup_data(annotation_dir)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build RoadGroup interaction-enhanced annotations for SIM")
    parser.add_argument("--image-root", required=True, help="Path to RoadGroup group image directory")
    parser.add_argument("--annotation-root", required=True, help="Path to RoadGroup annotation directory")
    parser.add_argument(
        "--pt-matrix",
    ...
    parser.add_argument(
        "--output-dir",
        default="",
        help="Directory to write RoadGroup_enhanced.pkl. Defaults to annotation root.",
    )
    args = parser.parse_args()

    output_dir = args.output_dir or args.annotation_root
    process_roadgroup_dataset(args.image_root, args.annotation_root, args.pt_matrix, output_dir)
