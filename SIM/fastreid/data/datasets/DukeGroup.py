...
#     def load_data(self):
#         """加载DukeGroup增强标注数据"""
#         with open(osp.join(self.annotation_dir, 'DukeGroup_enhanced.pkl'), 'rb') as f:
...
#
#         # 保存训练集
#         with open(osp.join(self.output_dir, 'DukeGroup_train.pkl'), 'wb') as f:
#             pickle.dump(train, f)
#
#         # 保存查询集
#         with open(osp.join(self.output_dir, 'DukeGroup_query.pkl'), 'wb') as f:
#             pickle.dump(query, f)
#
#         # 保存画廊集
#         with open(osp.join(self.output_dir, 'DukeGroup_gallery.pkl'), 'wb') as f:
#             pickle.dump(gallery, f)



@DATASET_REGISTRY.register()
class DukeGroup(ImageDataset):
    _junk_pids = [0, -1]
    dataset_dir = 'DukeGroup'
    dataset_name = "DukeGroup"
    ...
    def __init__(self, root='datasets', dataset_root='', image_root='',
        ...
        self.label_dir = label_root or osp.join(self.dataset_dir, 'annotations')

        # # 划分数据集（如果尚未划分）
        # splitter = DukeGroupSplitter(self.data_dir, self.label_dir, self.label_dir)
        # if not (osp.exists(osp.join(self.label_dir, 'DukeGroup_train.pkl')))and \
        #         not (osp.exists(osp.join(self.label_dir, 'DukeGroup_query.pkl')))and \
        #              not (osp.exists(osp.join(self.label_dir, 'DukeGroup_gallery.pkl'))):
        ...
        super(DukeGroup, self).__init__(train, query, gallery, **kwargs)

    def process_dir(self, dir_path, label_path, type):
        if type == 'train':
            labels = osp.join(label_path, f'DukeGroup_{type}.pkl')
            ...
            # DukeGroup使用相同的测试集作为gallery
            labels = osp.join(label_path, 'DukeGroup_gallery.pkl')
            with open(labels, 'rb') as f:
                labels = pickle.load(f)

        # elif type == 'gallery':
        #     # DukeGroup使用相同的测试集作为gallery
        #     labels_1 = osp.join(label_path, 'DukeGroup_query.pkl')
        #     with open(labels_1, 'rb') as f:
        #         labels_1 = pickle.load(f)
        #     labels_2 = osp.join(label_path, 'DukeGroup_gallery.pkl')
