...
from methods.SIM.fastreid.modeling.backbones import build_backbone
from methods.SIM.fastreid.modeling.heads import build_heads
from methods.SIM.fastreid.modeling.losses import *

from methods.SIM.fastreid.modeling.meta_arch.build import META_ARCH_REGISTRY
...
class Baseline(nn.Module):
    ...
    def forward(self, batched_inputs):
        images, images_p, layout, p_mask, img_names, avg_probs = self.preprocess_image(
            batched_inputs)
        # pdb.set_trace()
        if self.training:
            ...
            features, features_p = self.backbone(images, images_p, layout,
                                                 p_mask, n_t, n_c,
                                                 avg_probs=avg_probs)
    ...
    def preprocess_image(self, batched_inputs):
        ...
        if isinstance(batched_inputs, dict):
            ...
            avg_probs = batched_inputs.get('avg_probs')
            ...
            while p_index < length:
                p_mask[p_index:p_index +
                       batched_inputs['num_p'][n_index]] = p_value
                p_index = p_index + batched_inputs['num_p'][n_index]
                n_index = n_index + 1
                p_value = p_value + 1
            if 'img_paths' in batched_inputs:
                img_names = batched_inputs['img_paths']
        ...
        return images, images_p, layout, p_mask, img_names, avg_probs
    ...
    def losses(self, outputs_g, outputs_p, gt_labels_g, gt_labels_p):
        ...
        if 'CrossEntropyLoss' in loss_names:
            ...
            loss_dict['loss_cls_g'] = cross_entropy_loss(
                cls_outputs_g, gt_labels_g, ce_kwargs.get('eps'),
                ce_kwargs.get('alpha')) * ce_kwargs.get('scale')

            loss_dict['loss_cls_p'] = cross_entropy_loss(
                cls_outputs_p, gt_labels_p, ce_kwargs.get('eps'),
                ce_kwargs.get('alpha')) * ce_kwargs.get('scale')
        ...
        if 'TripletLoss' in loss_names:
            ...
            loss_dict['loss_triplet_g'] = triplet_loss(
                pred_features_g, gt_labels_g, tri_kwargs.get('margin'),
                tri_kwargs.get('norm_feat'),
                tri_kwargs.get('hard_mining')) * tri_kwargs.get('scale')

            loss_dict['loss_triplet_p'] = triplet_loss(
                pred_features_p, gt_labels_p, tri_kwargs.get('margin'),
                tri_kwargs.get('norm_feat'),
                tri_kwargs.get('hard_mining')) * tri_kwargs.get('scale')
