...
from methods.SIM.fastreid.layers import (
    IBN,
    SELayer,
    Non_local,
    get_norm,
...
class ResNet(nn.Module):
    ...
    def forward(self, imgs_g, imgs_p, p_mask):
        # only for some specical testing
        if imgs_p == None and p_mask == None:
            return self.forward0(imgs_g)

        # fea_g = self.forward0(imgs_g)
        fea_g = imgs_g
        fea_l = self.forward0(imgs_p)

        temp = []
        for i in range(fea_g.shape[0]):
            temp.append(fea_l[p_mask == i].mean(dim=0))

        # fea_g_a = (fea_g + torch.stack(temp, dim=0))/2
        fea_g_a = torch.stack(temp, dim=0)
        return fea_g_a, fea_l
