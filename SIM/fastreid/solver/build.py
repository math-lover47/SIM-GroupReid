...
from methods.SIM.fastreid.config import CfgNode
...
def _generate_optimizer_class_with_freeze_layer(
    ...
    *,
    ...
    assert (freeze_layers is not None and freeze_iters
            > 0), "No layers need to be frozen or freeze iterations is 0"
...
def build_optimizer(cfg, model):
    ...
    if solver_opt == "SGD":
        ...
        return maybe_add_freeze_layer(
            cfg,
            maybe_add_gradient_clipping(cfg, getattr(
                torch.optim, solver_opt)))(
                    params,
                    lr=cfg.SOLVER.BASE_LR,
                    weight_decay=cfg.SOLVER.WEIGHT_DECAY,
                )
    ...
    for module_name, module in model.named_modules():
        for module_param_name, value in module.named_parameters(recurse=False):
            ...
            if module_name.split('.')[0] == "heads" and (
                ...
                hyperparams["lr"] = hyperparams.get("lr",
                                                    base_lr) * heads_lr_factor
            params.append({
...
def build_lr_scheduler(cfg, optimizer, iters_per_epoch):
    ...
    scheduler_dict["lr_sched"] = getattr(
        lr_scheduler, cfg.SOLVER.SCHED)(**scheduler_args[cfg.SOLVER.SCHED])
